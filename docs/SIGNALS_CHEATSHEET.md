...
// Validación reactiva
const isEmailValid = calc(() => {
  const email = loginForm.email.get();
  return email.includes("@") && email.length > 5;
});

const isPasswordValid = calc(() => {
  return loginForm.password.get().length >= 8;
});

const canSubmit = calc(() => {
  return isEmailValid.get() && isPasswordValid.get();
});

// Sincronizar con localStorage
monitor(() => {
  const shouldRemember = loginForm.rememberMe.get();
  if (shouldRemember) {
    const email = loginForm.email.get();
...
function LoginForm() {
  return (
    <form>
      <input
        type="email"
        value={loginForm.email.get()}
        onChange={(e) => loginForm.email.set(e.target.value)}
      />
      {!isEmailValid.get() && <span>Email inválido</span>}

      <input
        type="password"
        value={loginForm.password.get()}
        onChange={(e) => loginForm.password.set(e.target.value)}
      />
      {!isPasswordValid.get() && <span>Mínimo 8 caracteres</span>}

      <button disabled={!canSubmit.get()}>Entrar</button>
    </form>
  );
}
```
...
const cantidadItems = calc(() => {
  const items = carrito.get();
  return items.reduce((sum, item) => sum + item.cantidad, 0);
});

// Agregar producto
function agregarProducto(nombre: string, precio: number) {
...
function Carrito() {
  return (
    <div>
      <h2>Carrito ({cantidadItems.get()} items)</h2>

      {carrito.map((itemSignal, index) => {
...
### Ejemplo 3: Buscador con Filtros
...
const todosLosProductos = stateArray(() =>
  state({
    nombre: "",
    categoria: "",
    precio: 0,
...
const categoriaSeleccionada = state("todas");
const precioMaximo = state(1000);

// Productos filtrados (se actualiza automáticamente)
const productosFiltrados = calc(() => {
  const productos = todosLosProductos.get();
  const texto = busqueda.get().toLowerCase();
  const categoria = categoriaSeleccionada.get();
  const maxPrecio = precioMaximo.get();
...
const cantidadResultados = calc(() => productosFiltrados.get().length);
  ...
  return (
    <div>
      <input
        ...
        placeholder="Buscar..."
        value={busqueda.get()}
        onChange={(e) => busqueda.set(e.target.value)}
      />

      <select
        value={categoriaSeleccionada.get()}
        onChange={(e) => categoriaSeleccionada.set(e.target.value)}
      >
      ...
      <input
        ...
        min="0"
        max="1000"
        value={precioMaximo.get()}
        onChange={(e) => precioMaximo.set(parseInt(e.target.value))}
      />

      <p>{cantidadResultados.get()} resultados</p>

      {productosFiltrados.get().map((producto, i) => (
